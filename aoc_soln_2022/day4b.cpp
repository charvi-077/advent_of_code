...
int main() {
    ...
    if (newfile.is_open()) {
        ...
        while (getline(newfile, num_txt)) {
            ...
            if ((num_vec[0] >= num_vec[2] && num_vec[0] <= num_vec[3]) ||
                (num_vec[1] <= num_vec[3]) && num_vec[1] >= num_vec[2]) {
                count++;
                std::cout << "yes" << std::endl;
            } else if ((num_vec[2] >= num_vec[0] && num_vec[2] <= num_vec[1]) ||
                       (num_vec[3] <= num_vec[1]) && num_vec[3] >= num_vec[0]) {
                count++;
                std::cout << "yes" << std::endl;
            }
        }
